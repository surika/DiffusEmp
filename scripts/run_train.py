...
if __name__ == '__main__':
    ...
    parser.add_argument('--use_visible_mask', type=bool, default=False)
    ...
    parser.add_argument('--hostname', type=str, default='', help='host name')
    ...
    Model_FILE = f"diffusemp_{args.dataset}_h{args.hidden_dim}_lr{args.lr}" \
                f"_t{args.diff_steps}_{args.noise_schedule}_{args.schedule_sampler}" \
                f"_seed{args.seed}_{args.hostname}"
    if args.notes:
    ...
    COMMANDLINE = f" OPENAI_LOGDIR={Model_FILE}  " \
                  ...
                  f"--lr {args.lr} " \
                  f"--use_visible_mask {args.use_visible_mask} " \
